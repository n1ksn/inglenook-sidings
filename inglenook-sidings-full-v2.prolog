...
%% intermediate state [_,Trk1List1,_,_], where Trk1List1 is the last 3
...
solve_fwd(StartState, EndState, Path) :-
  ...
  nth1(2, EndState, Trk1List),
  (last_n(3, Trk1List, Trk1List1, _); Trk1List1 = Trk1List),
  IState = [_, Trk1List1, _, _],
  path(StartState, IState, Path1), !,      % Get path to intermediate state
  [_|Path1Tail] = Path1,          % Strip intermediate goal state
  path(IState, EndState, Path2), !,      % Get path from intermediate goal state
...
%%% Finds both forward and reverse solutions connecting StartState and
%%% EndState and sets Path as the shorter of the two (or the forward
